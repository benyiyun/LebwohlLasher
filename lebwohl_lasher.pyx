...
def all_energy(cnp.ndarray[double, ndim=2] arr, int nmax):
    ...
    return enall  # This is safe because enall is a double (not a Python object)
...
def MC_step(cnp.ndarray[double, ndim=2] arr, double Ts, int nmax):
    ...
    for i in prange(nmax, nogil=True):  # Parallel execution
        for j in range(nmax):
            ...
            en0 = one_energy(arr_view, ix, iy, nmax)  # Now works in nogil mode
            arr_view[ix, iy] += ang
            en1 = one_energy(arr_view, ix, iy, nmax)  # Now works in nogil mode
